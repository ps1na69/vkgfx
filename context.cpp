...
void Context::createInstance(const ContextConfig& cfg) {
    // volk: load vulkan-1.dll and vkGetInstanceProcAddr before any Vulkan call
    VkResult volkRes = volkInitialize();
    if (volkRes != VK_SUCCESS)
        throw std::runtime_error("[vkgfx] volkInitialize() failed -- is a Vulkan-capable GPU driver installed?");
    ...
    // volk: load all instance-level Vulkan function pointers
...
void Context::createLogicalDevice(const ContextConfig& cfg) {
    ...
    // volk: load all device-level Vulkan function pointers
...
void Context::createVMA() {
    // volk: give VMA the proc-address getters so it can resolve Vulkan
    // functions through volk's dispatch table instead of linking vulkan-1.lib.
