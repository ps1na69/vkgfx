...
public:
    ...
    static std::shared_ptr<Mesh> createSphere(float radius = 1.f, uint32_t sectors = 32, uint32_t stacks = 16);
    ...
    void setMaterial(std::shared_ptr<Material> mat, uint32_t slot = 0);
    [[nodiscard]] std::shared_ptr<Material> getMaterial(uint32_t slot = 0) const;
    [[nodiscard]] size_t materialCount() const { return m_subMeshes.size(); }
