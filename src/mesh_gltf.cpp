...
static std::shared_ptr<Texture> loadTexture(
    ...
    std::optional<fastgltf::TextureInfo> texInfo,
    TextureCache&                    cache,
    const std::filesystem::path&     baseDir,
    TextureDesc                      desc)
{
...
std::vector<std::shared_ptr<Mesh>> Mesh::loadGLTF(const std::string& path,
    ...
    for (const fastgltf::Material& gltfMat : asset.materials) {
        ...
        mat->setAlbedo(glm::vec4(bc[0], bc[1], bc[2], bc[3]));
        ...
        float emStrength = 1.0f;
        if (gltfMat.emissiveStrength.has_value())
            emStrength = gltfMat.emissiveStrength.value();
        mat->setEmissive(glm::vec4(em[0], em[1], em[2], emStrength));
        ...
        if (albedoTex) mat->setAlbedoTex(albedoTex);
